    ...
    # handles other excel format.
            ...
            try:
                ...
                for row in listOfInfo:
                    print("Adding team")
                    returnVal = AddTeam(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], row[11], row[12], row[13], row[14], row[15], row[16], row[17], row[18], Data, row[20])
                    if returnVal is -1:
...
def Search(value, Data, listOfEntries):
    ...
    listOfEntries[19].set(str(Data.getItem("control_type")))
